...
g_notify_to_emails="aceway@qq.com,iuway@qq.com,upython@qq.com"
g_from_email="upython@qq.com"
g_from_email_pwd="*********"
g_smtp_server="smtp.exmail.qq.com"
g_smtp_port=465
...
def submmit_info(the_url=None, params={}):
    ...
    try:
        if the_url is None or not isinstance(params, dict):
            ...
            urlfile= urllib.urlopen(the_url, params)
            result = urlfile.read()
    except Exception, e:
        result['code'] = -1
...
def notify_wan_ip_with_httpsvr(http_server="http://members.3322.org/dyndns/getip", tmp_ip_file="/tmp/.WAN_IP_ADDR"):
    import urllib
    import os
    ipInfo = urllib.urlopen(http_server)
    new_ip = ipInfo.read().strip()
    str_msg = "http://%s:%d/ \n\nssh -p20131 %s \n"  %(new_ip, g_port, new_ip)
    if os.path.exists(tmp_ip_file):
        with open(tmp_ip_file, 'r') as fp:
            old_ip = fp.read().strip()
        if g_debug: print "old ip:", old_ip, "new ip:", new_ip
        if old_ip.strip() == new_ip.strip():
            ...
            #submmit_info(the_url=g_url, params={'token':g_token, 'port':g_port})
            #send_file_to_email("THE NEW IP", tmp_ip_file, g_notify_to_emails, g_from_email, g_from_email_pwd, g_smtp_server, g_smtp_port)
    else:
        with open (tmp_ip_file, 'w') as fp:
            fp.write(new_ip)
